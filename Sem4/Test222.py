...
class SplashScreen:
    def __init__(self, root):
        ...
        self.root.geometry("1000x1000")  # Adjusted size for better fit
        self.root.overrideredirect(False)  # Hide the title bar

        # Load Background Image (background2.jpg)
        try:
            bg_main = Image.open("background2.jpg")
            bg_main = bg_main.resize((800, 500))  # Resize to fit window
            self.bg_main_image = ImageTk.PhotoImage(bg_main)

            bg_main_label = Label(self.root, image=self.bg_main_image)
        ...
        try:
            ...
            self.bg_top_image = ImageTk.PhotoImage(bg_top)

            bg_top_label = Label(self.root, image=self.bg_top_image)
            bg_top_label.place(relx=0.5, rely=0.0, anchor="n")  # Set image at the top
        except FileNotFoundError:
            print("College name image not found.")

        # Display Project Name
        self.project_label = Label(self.root, text="", font=("Arial", 30, "bold"), bg="white", fg="black")
        self.project_label.place(relx=0.5, rely=0.85, anchor="center")

        # Animate text appearance
        self.fade_in("Project Name: Echo Box !!!")

        # Bind the Enter key to close the splash screen
        self.root.bind("<Return>", self.close_splash)  # Bind Enter key

    def fade_in(self, text, index=0):
        if index < len(text):
            self.project_label.config(text=text[:index + 1])  # Display characters gradually
            self.root.after(100, self.fade_in, text, index + 1)

    def close_splash(self, event=None):  # Accept an event parameter for the key binding
        self.root.destroy()  # Close the splash screen

    def show(self):
        self.root.mainloop()

# Example of how to use the SplashScreen class in your main application
if __name__ == "__main__":
    splash_root = tk.Tk()
    splash = SplashScreen(splash_root)
    splash.show()  # Show the splash screen

    # After the splash screen, start the main application
    app = EmailOrganizerGUI()  # Assuming this is your main application class
